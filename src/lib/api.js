...
// MediScan API functions
export const mediscanApi = {
  // Upload a document for OCR processing
  uploadDocument: async (file, patientId = null) => {
    try {
      ...
      const response = await apiRequest('/individual/mediscan/ocr', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  getOcrJob: async (jobId) => {
    try {
      console.log(`Checking OCR job status: ${jobId}`);
      const response = await apiRequest(`/individual/mediscan/ocr/${jobId}`);
      console.log('OCR job status:', response);
      return response;
    } catch (error) {
      console.error(`Error checking OCR job ${jobId}:`, error);
      throw error;
    }
  },
  
  // Get all OCR jobs for the current user
  getOcrJobs: async (page = 1, perPage = 10) => {
    try {
      console.log(`Fetching OCR jobs (page ${page}, perPage ${perPage})`);
      const response = await apiRequest(`/individual/mediscan/ocr?page=${page}&per_page=${perPage}`);
      console.log('OCR jobs:', response);
      return response;
    } catch (error) {
  ...
  // Debug credentials
  debugCredentials: async () => {
    try {
      console.log('Checking OCR credentials');
      const response = await apiRequest('/individual/mediscan/debug/credentials');
      console.log('Credentials debug info:', response);
      return response;
    } catch (error) {
  ...
  // List documents (visits) for a patient
  listDocuments: async (patientId) => {
    return apiRequest(`/individual/mediscan/patient/${patientId}/documents`);
  },

  // Delete a specific document
  deleteDocument: async (patientId, visitId) => {
    return apiRequest(`/individual/mediscan/patient/${patientId}/documents/${visitId}`, {
      method: 'DELETE',
    });
  },

  // Update document
  updateDocument: async (patientId, visitId, data) => {
    return apiRequest(`/individual/mediscan/patient/${patientId}/documents/${visitId}`, {
      method: 'PUT',
